...
their tag bit.
Similarly, when \texttt{XLEN} is 64, we can use the anticipated 128-bit
...
CHERI-RV32 can reuse these opcodes in capability-encoding mode for
capability loads and stores (\insnnoref{C.CLC} and \insnnoref{C.CSC}).
Similarly, CHERI-RV64 can reuse the opcodes for floating-point double
loads and stores (\insnnoref{C.FLD} and \insnnoref{C.FSD}) in
capability-encoding mode for capability loads and stores.  The same
rules would apply to stack-relative memory access instructions.
