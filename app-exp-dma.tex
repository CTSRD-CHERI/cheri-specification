...
\begin{itemize}
	\item we carry around the address space stored as bits inside the capability, or
	\item we maintain the color as external metadata that is supplied to wherever a capability needs to be checked, or
	\item we arrange such that structurally there is no ambiguity at the point of check as to which address space a capability refers.
\end{itemize}

In the first case, bits that travel within the in-memory capability are used to identify its address-space identifier (ASID), such as a 'color' (see section~\ref{FIXME}).
...
Ideas on this topic are developed further in section \ref{FIXME}.
