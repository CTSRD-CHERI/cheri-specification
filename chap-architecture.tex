...
\item
  For larger allocations the base and top must be aligned to $2^{E+3+C}$ byte
  boundaries (i.e., the $E + 3 + C$ least significant bits are zero) where $E$
  is determined from the length, $l$, by
  $E = 52 - \text{CountLeadingZeros}(l[64:13])$, and $C$ is an additional carry
  bit from rounding up the truncated (to $E + 3 + MW - 4$ bits) length to a
  multiple of $2^{E+3}$ (that is, $C = 1$ if and only if any of the $E + 3$
