...
mode, \RSP{} is always used as the stack pointer. We will allow selecting \RSP{} or \CSP{} using the alternate-addressing-mode prefix (0x07).
For example, hybrid code generation in 64-bit long mode could add the 0x07 prefix before \insnnoref{PUSH} and \insnnoref{CALL} instructions to maintain distinct bounds for the stack in \CSP{} and integer data references in \DDC{}.
...
\begin{itemize}
  \item \insnnoref{MOVC} would handle memory loads and stores of
    capabilities as well as copies of capabilities between registers.
    ...
    \insnnoref{MOVC} performs the function of \insnref{CLC} and
    \insnref{CSC} instructions from CHERI-RISC-V which load and store
    capabilities respectively, as well as \insnref{CMove}
    which moves capabilities between registers.
  ...
  \item \insnnoref{ADDC} and \insnnoref{SUBC} would be used to adjust
    the offset of a capability similar to the \insnref{CIncOffset}
    instruction in CHERI-RISC-V.
    For these instructions, the source operand would be an
    integer whose value
...
\end{verbatim}
    ...
    For the register to register variants of the \insnnoref{ADDC}
    instruction, we will also support using the
    integer register operand as the capability destination register.
    For this instruction, we suggest the following example assembly
    representation:
...
\end{verbatim}
    ...
    This emphasizes the width difference between \CAX{} and \RAX{},
    though the destination and the second operand would be required
    to map to the same register number.
    The two versions of \insnnoref{ADDC} would be distinguished by the
    two opcodes \texttt{01} (First register operand is the capability
    operand and destination, and the second is the integer register)
    and \texttt{03} (First register operand is the integer operand
    and the capability destination and the second is the capability
    operand).
...
\begin{itemize}
  ...
  \item \insnref{CCSeal} rca, r/mc, rcb
    ...
    will be set equal to \emph{r/mc}.  Set \texttt{ZF} in \RFLAGS{} to 1 if
    \emph{r/mc} is sealing is successful or 0 otherwise; set the rest
    of \RFLAGS{} according to the resulting \emph{r/mc}.\textbf{otype}..
...
\subsubsection{Compartment-Reletive Pointer Instructions}
...
\begin{itemize}
  ...
  \item \insnref{CClearTags} m
    ...
    Clear the capability tags for a range of memory.
    \insnref{CClearTags} might be encoded as \insnnoref{REP MOVSB} with
    the same source and destination.
    Bounds and permissions would be enforced as usual for this encoding.
    Data would be unchanged, but tags would be cleared for all capabilities
    overlapped by the range specified by the source/destination address in
    \CSI{}/\CDX{} and the length in \RCX{}.

    As with \insnnoref{REP MOVSB}, this instruction may raise either a
    Capability Violation Fault (see Section~\ref{sec:x86:capability-fault})
    or a Page Fault.
...
We propose that vector registers should preserve tags for simple
memory operations, and clear tags for any arithmetic operation.
Loads of
vector registers should preserve a tag for contiguous, aligned
128-bit words, and stores of vector
registers to memory should preserve tags if the store is aligned
and there had been no intervening change to the register.
Existing vector
instructions which manipulate vector register contents will clear
all tags in the destination register.

\subsubsection{Memory Copies}

In CHERI C, \ccode{memcpy()} must preserve tags.
Vector loads and stores are often used to implement \ccode{memcpy()}.
Another option may be to support an
...
aliases over time such as the \FSBASE{} and \GSBASE{} MSRs and the
...
the address field of the \CFS{}, \CGS{}, and \KGS{} capabilities,
respectively.  Writes to these MSRs should set the address field of the
...
return the address field of the \CFS{} and \CGS{} capabilities,
respectively.  The \insnnoref{WRFSBASE} and \insnnoref{WRGSBASE}
instructions should set the address field of the respective capability
