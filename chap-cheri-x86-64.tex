...
CET also includes guarantees on speculative execution for control flow
transitions that are protected by CET assertions.

\subsubsection{Compatability with CHERI-x86-64}
...
CHERI-x86-64 does not replicate the behavior of the shadow stack, and the
shadow stack should be able to run with CHERI-x86-64 executables, though
shadow stack pointer MSRs should need to be extended to be capabilities.

\subsubsection{Synergy with CHERI-x86-64}
...
The shadow stack implementation might be extended to hold capabilities,
...
\subsection{AMD SME and SEV, and Intel TME and MKTME}
