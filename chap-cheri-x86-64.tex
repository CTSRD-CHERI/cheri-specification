...
However, there are also some key implementation differences that, in aggregate, have limited the utility of x86 segments to the extent that they have been nearly deprecated in x86-64.

First, x86 segment descriptors do not contain an offset, so a full reference requires both a segment descriptor and an integer offset.
Secondly, x86 segment descriptors cannot be manipulated by the user program, but are selected from a size-limited table that is populated by privileged code.
As a result of these two factors, segment descriptors are not suitable to enforce memory safety for program objects.
...
Due to this limitation, x86 segmentation is typically not used in modern operating
