...
Therefore all ME technologies, even MKTME, fit neatly below CHERI enforcement
...
Intel's Memory Protection Extension (MPX) was designed to accellerate
bounds checking with various usage models.
MPX shipped comercially in 2015 with Skylake, the only high-end
...
MPX introduced instructions to create bounds and compare integer
values to the bounds.
Upper and lower bounds are compared separately with dedicated
instructions.

MPX also introduces instructions to move four new BND registers
between one another and to and from memory.

MPX further introduces two table access instructions
for loading and storing bounds for any pointer in memory
from a sparse table.
Each entry in the table is 32-bytes; 8 bytes for each of the bounds
and 8-bytes to hold a copy of the expected pointer value.
Using these instructions, a program can be compiled to stash
bounds for each pointer in memory in a shadow space, loading
and storing them with the pointers themselves.
...
MPX was designed to maximise compatibility and ease of use.
...
With no hard security guarantees and performance that was
barely faster than similar software schemes, MPX adoption
...
compatability, CHERI-x86-64 must resist the pressure
to cleverly maintain compatability in a way that creates a
security loophole.

In addition, CHERI-x86-64 implementators should resist the pressure
