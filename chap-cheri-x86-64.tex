...
Secondly, x86 segment descriptors cannot be manipulated by the user program, but are stored to a size-limited table by privileged code.
As a result of these two factors, segment descriptors are not suitable to implement references to program objects.
Indeed, x86 segments were thought to describe coarse-grained program objects; of the six segment selector registers, three are reserved for code, stack, and general data accesses, and a fourth register is typically used to define the location of thread-local storage (TLS).

Due to these limitations, x86 segmentation is typically not used in modern operating
...
We propose to implement x86-CHERI capabilities in the
style of the other CHERI architectures, for example, extending
general-purpose integer registers to hold capabilities.
While we will generally use the language of the CHERI architecture,
we will also endeavor to allow for an architectural
description in terms of segment pointers as an extension of x86 segmentation, which may integrate more naturally into a future x86 specification.
...
\begin{itemize}
 \item \DDC{}, which transforms and controls memory operands using
   ...
   architectural segments. \CFS{}, \CGS{} constrain FS and GS accesses
