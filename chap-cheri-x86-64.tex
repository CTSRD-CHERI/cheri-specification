...
These features are superficially similar to features in CHERI capabilities.
...
As a result of these two factors, x86 segments are not efficient for use as general-purpose pointers, which is the primary use case for CHERI capabilities.
Indeed, x86 segments were thought to describe coarse memory regions; of the six segment selector registers, three are reserved for code, stack, and general data accesses, and a fourth register is typically used to define the location of thread-local storage (TLS).
...
Software for x86 systems implement pointers as virtual addresses in general-purpose registers directly, ignoring segmentation.

We propose to implement x86-CHERI capabilities in the style of pointers, as with all other CHERI architectures, leaving behind privileged table-based segment management.
Nevertheless, we will also endeavor to integrate with x86 segmentation to the extent that CHERI-x86-64 could be be described as an improvement to x86 segmentation in order to fit more naturally into a future x86 specification.
