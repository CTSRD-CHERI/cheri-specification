...
\item Capability values should be intentionally used.  Instructions
  should explicitly specify if a register operand should be used as a
...
\subsection{Capability Registers and Segment Tables}

The x86 architecture first added virtual memory support via
relocatable and variable-sized segments with a
mask of permissions.  Each memory reference resolved with respect to a
segment for relocation from its base address, bounds
checking, and access checks.  Special segment types permitted transitions
to and from different protection domains.

These features are superficially similar to CHERI capabilities.
However, x86 segment manipulation and use do not match modern programming models to the extent that they fell out of use, were not supported by modern operating systems, and were nearly deprecated in the x86-64 architectural extension.

Firstly, x86 segment descriptors cannot be manipulated by the user program, but are selected from a size-limited table that is populated by privileged code.
Modern programming models expect objects to be created and destroyed in userspace, for example in malloc, and do not limit the number of objects beyond the available memory.
Secondly, x86 segment descriptors do not contain an offset, so a full reference requires both a segment descriptor and an integer offset.
This increases the cost of using segment references and motivates aggregating objects into a very few segments.
Thirdly, only six x86 segment-selectors are currently supported, with dedicated prefixes for selecting each segment; four selectors have dedicated uses, and increasing the number of segment selectors is very expensive from an encoding perspective.
This optimizes for a small number of in-use segments, further motivating object aggregation and discouraging use as general-purpose references.

On the 32-bit x86 architecture, operating systems generally create
...
addresses only for two segments, and these through a configuration register rather than through the descriptor table.
Software for x86 systems implement pointers as virtual addresses in general-purpose registers, ignoring segmentation.

In contrast to x86 segments, CHERI capabilities are optimized for use as pointers.
We propose to implement x86-CHERI capabilities as an extension of general-purpose registers, as with other CHERI architectures, leaving behind privileged table-based segment management.
Nevertheless, we will also endeavor to integrate with x86 segmentation to the extent that CHERI-x86-64 could be be described as an extension of x86 segmentation, fitting more naturally into a future x86 specification.
...
\item \CIP{} transforms and controls program-counter-relative (i.e. Code Segment) fetches.
...
\begin{itemize}
 \item \DDC{}, which transforms and controls memory operands using
   integer addresses, is split into registers that constrain the
   x86 architectural segments. \CFS{}, \CGS{} constrain FS and GS accesses
   respectively (aliasing \FSBASE{} and \GSBASE{}), and \CXS{} constrains
   \CS{}, \DS{}, \ES{}, and \SS{}.
...
A new capability register will be required to constrain integer offsets
to the DS, SS, and ES segments, fulfilling the function of \DDC{}.
We recommend calling this register \CXS{}.

The x86-64 architecture uses the \FS.base{} and \GS.base{} model-specific registers (MSRs) to
provide an offset for accesses through the \FS{} and \GS{} segments which are commonly used for
...
These additional registers will be exposed as a new bank of special registers
that can hold capabilities.
As with other x86 register banks such as
model-specific registers and debug registers, additional capability registers
cannot be used
as general-purpose operands in existing instructions, though various instruction behaviors may be affected by their value.
...
A note on style: \CIP{} generally replaces the code segment descriptor (\CS{}), and the few functional fields of \CS{} that remain in x86-64 can be viewed as belonging with the \cflags{} field of \CIP{}, which allows unprivileged manipulation for convenience.
Storing these legacy fields in the segment table rather than in code capabilities is awkward, but traditional and avoids complicating the capability format.
...
instructions can toggle between capability addressing and segment-relative
addressing via the 0x07 opcode prefix. 
32-bit or 64-bit segment-relative addressing will be treated as offsets
into the capability associated with the defined segment: \CFS{} for \FS{}; \CGS{} for \GS{}; \CXS{} for \CS{}, \DS{}, \ES{}, or \SS{}.
Instructions using capability-aware addressing
would always use 64-bit virtual addresses.
A note on style: capability addressing could be viewed as extending segmentation to allow segment descriptors to be carried in-line with the pointer rather than indirected through a table.
...
Near branches would be extended to support capability operands.
In 64-bit long mode, a capability operand prefix would select a
capability operand size.  In capability mode, near branches would
only support capability operands.
Relative near branches would always modify the offset of the \CIP{}
capability regardless of operand size.
Near branches which use an integer operand would set the offset of the
\CIP{} capability while near branches using a capability operand would
