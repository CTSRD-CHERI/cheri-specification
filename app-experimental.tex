...
In this extension, we propose to split the cisentryj (CHERI indirect sealed entry jump) instruction into two sub instructions: Lil (load isentry linked) and cjaurl (CHERI jump and unseal into register linked).
...
The lil instruction loads a capability through an isentry capability and clears its tag.
In the subsequent instruction this capability can be picked up by a cjaurl, which then jumps to this code capability and unseals the isentry into ct6.
The two instructions are linked and all security properties are held as explained in the following paragraphs.

If attackers executed only the lil instruction, this will leave them with an untagged code capability.
This code capability cannot be used in regular jumps because it is untagged.
The only way it can be used for a jump is in a subsequent cjaurl instruction.

An attacker can only jump to a code capability with the isentry it came from.
This can be divided into two cases: If the instruction following the lil is a cjaurl, this will jump to the code pointed to by the code capability and unseal the isentry.
The register holding the isentry is stored in the reservation information of the lil instruction.
In the second case, if the instruction following the lil is not a cjaurl, the reservation will have become invalid.
...
It is important to note that the linked pair of a lil and cjaurl instruction does not give any guarantees about memory consistency. Memory consistency is not an issue tackled by CHERI, but needs to be addressed by other architectural mechanisms, e.g. atomic memory operations.

\subsubsection{Allowing more instructions in between lil and cjaurl}

In order to enhance performance, we want to allow for ordinary instructions to appear between a lil and cjaurl instruction.
This could allow for the load-to-use penalty to be reduced.

The invariants laid out above still need to hold: One must only jump to a linked code capability with a cjaurl instruction.
...
The lil instruction makes a reservation, which stores the following information: a one bit tag whether the reservation is valid, a five bit index to indicate the register of the isentry.
...
A lil instruction is a write to the ct6 register and thus invalidates the reservation.
...
In this proposal there is no maximum number of instructions between the lil and cjaurl instruction.
...
instruction into a pair of linked instructions.
