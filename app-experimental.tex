...
The current RV64Y levels proposal, as well as the Morello and ISAv9 architectures introduce a single bit Local/Global mechanism to control the flow of capabilities in a program.
Furthermore, the RV64Y draft proposes a scheme to extend capability levels to multiple bits, thereby extending the CL and SL fields to N bits, where N is the number of bits required to encode the level.
...
\item If this extension is not implemented, the bits taken by this extension have to be all “1”s for the FA, CL, SL, and EL fields. For the F bit the value is “0”. This leads to software producing global capabilities (can be stored everywhere) and fully-permissive capabilities (can store all capabilities). Therefore, software that is oblivious to capability levels runs without any limitation to the flow of capabilities.
