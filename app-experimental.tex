...
At minimum, we would like to be able to have an independent level interpretation for the four RISC-V rings (machine mode, hypervisor mode, supervisor mode, and user mode), which would already result in 8 bits for CL and SL (4 * 1 bits for CL, 4 * 1 bits for SL).
...
As previously noted, we make the assumption that capabilities in Protection Level Pi are always more sensitive than capabilities in $P_{i+1}$.
This seems to be a sensible assumption if we associate architectural privilege levels (rings) to the protection level.
...
The F bit affects the propagation semantics of capabilities by modifying the  store-side rule is modified such that the tag is cleared under the following condition, where Fdata and CLdata are the respective bit fields of the data capability to be stored, and $F_{auth}$ and $S_{Lauth}$ are the respective bit fields of the capability authorising the capability store:

$\{F_{data}, CL_{data}\} < \{F_{auth}, neg(S_{Lauth})\} \Rightarrow  t := 0$
...
\item Setting F = 1 makes a capability “super global” (i.e. as shareable as an F = 0 CL=-1 capability) regardless of its CL.
\end{enumerate}
