...
\end{itemize}
\end{itemize}
\item Change to the following instructions:
\begin{itemize}
...
\item In the case of the CL and SL fields monotonicity means that the value can only ever be arithmetically decreased, but never increased.
\item A capability has its tag cleared on a store if $\{F, CL\} < \{F, neg(SL)\}$; this leads to the property that foreign capabilities can always be stored by current capabilities.
...
\item \textbf{Transitive load} -- If a capability load is authorised by a capability with the F bit set the loaded capability will have the F bit also set (This the forward transitivity rule).
