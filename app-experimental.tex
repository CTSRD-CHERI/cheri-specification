...
Furthermore, the RV64Y architecture proposes a scheme to extend capability levels to multiple bits, thereby extending the CL and SL fields to N bits, where N is the number of bits required to encode the level.
...
\begin{enumerate}
\item Have different capability levels interpretations within the same privilege level. E.g. A V8 compartment uses \textit{local} for javascript allocations and \textit{global} for non-javascript objects vs a third party library that uses \textit{local} for the stack and \textit{global} for the heap.
