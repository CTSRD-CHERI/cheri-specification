...
\begin{enumerate}
  \item CLILC: CHERI load isentry linked code: this instruction has two operands. The register index of the ``points to pair'' isentry and a destination register index.
  The instruction loads the capability at the address the isentry is pointing and stores it in the destination register.
  The loaded capability is sealed with the 
  ...
  \end{itemize}
  \item CLILD: CHERI load isentry linked code; this instruction has one operand, which is the register of the ``points to pair'' isentry.
  ...
  \begin{itemize}
    ...
    \item Check whether the isentry can load a CAP-WIDTH wide word.
  \end{itemize}
  \item CJAURLP: CHERI jump and unseal into register linked pair; this instruction has one three operands of which one is explicit and the other two are implicit.
