...
When using sentries (sealed entry capabilities) to call into trusted code, the same piece of trusted code can be called from multiple threads and each thread must have a distinct trusted data capability.
...
\begin{enumerate}
\item The most simple design is to have one thread in the system called the Supervisor Thread ID (STID).
      The register is a CSR that is only writable from S mode or more privileged modes if the ASR permission bit is set.
...
Thus, we conclude that the reading speed of the TID register matters for overall performance whereas the writing speed does not.
We envision that this strategy might be useful to other pure-data CSRs.
