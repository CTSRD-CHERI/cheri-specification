...
\subsubsection{Instruction sequence for CBuildCap unsealing}
...
Sealed keys can be represented in multiple ways in the capability format.
Some of the choices have an impact on other properties. (see Section~\ref{subsubsec:sealedkey_subtyping}).
As a first option, the sealed key can be an extension to the current seal bit, bringing it to 2 bits in total.
This would lead the sealed key itself being sealed and thus it cannot be manipulated.

Second, the permissions can be extended by one bit for sealed keys.
...
Third, the format can include a mode bit, which tells whether this is a sealed key capability.
This allows for manipulations on the capability.
The sealed key bit can never be cleared, but every non-sealed-key capability can become a sealed key.
This gives code the power to create their own unsealers with needing to have given an unsealer before.

\subsubsection{Subtyping}
\label{subsubsec:sealedkey_subtyping}

Software needs to option to pass a subset of its unsealing capabilities to other software.
Therefore, sealed keys need to be manipulated.
A sealed key must never be converted back to a ``conventional capability''.
This would allow access to capabilities without ever having received the actual sealed capability.
When creating a sealed key, the capability cannot be sealed.
Otherwise, one could create a sealed key for a sealed capability.

We allow the following manipulations on sealed keys:
...
One is laid out above with using a sealed key through an invocation of \texttt{cbuildcap}.
This would enforce the sealed key to be a superset of the capability to be unsealed.

However, we also envision that it might be useful to software to have a sealed key capability that successfully unseals if the ``sealed key bit'' is set and the bounds of the sealed key span at least the capability to be unsealed.

\subsubsection{Comparison with otypes}

The sealed key approach conflates the object type space and the address space into one space.
In the otype space, two capabilities can be sealed by the same otype.
Furthermore, these capabilities are not bound to point to contiguous addresses.
With sealed keys, the address of the capability becomes its ``type'' and thus two capabilities can never have the same ``type''.
Otherwise, one of the two capabilities would be a subset of the other capability.
